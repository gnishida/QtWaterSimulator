...
void OBJLoader::load(const char* filename, std::vector<Vertex>& vertices)
{
	...
	while (getline(file, line)) {
		if (line.substr(0, 2) == "v ") {
			...
			int index2 = lastIndexOfNumberLetter(line);
			std::vector<std::string> values = split(line.substr(index1, index2 - index1 + 1), ' ');
			raw_texCoords.push_back(atof(values[0].data()));
			raw_texCoords.push_back(atof(values[1].data()));
	...
	for (int i = 0; i < v_elements.size(); i+=3) {
		...
		if (t_elements.size() > 0) {
			...
			vertices[i].texCoord[0] = 0.0; vertices[i].texCoord[1] = 0.0;
			vertices[i+1].texCoord[0] = 1.0; vertices[i+1].texCoord[1] = 0.0;
			vertices[i+2].texCoord[0] = 0.0; vertices[i+2].texCoord[1] = 1.0;
		}
